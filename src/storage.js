...
if (typeof window !== "undefined") {
  if (!supabase) {
    console.warn("%c[Smaashallah] Supabase NOT configured in this build.", "color:#e0a100;font-weight:bold");
    ...
    if (rawUrl && rawUrl.trim() !== url) {
      console.warn("[Smaashallah] Your VITE_SUPABASE_URL had extra characters and was auto-trimmed to:", url);
    ...
    console.info("[Smaashallah] Supabase URL:", url);
    console.info("[Smaashallah] Key type:", kind, "| length:", apiKey.length, "chars");
  }
}
...
export async function kvGet(key) {
  ...
  if (error) { console.error("[Smaashallah] READ failed:", error.message, error); throw error; }
...
export async function kvSet(key, value) {
  ...
  if (error) { console.error("[Smaashallah] WRITE failed:", error.message, error); throw error; }
}
...
export async function uploadPhoto(file, prefix = "match") {
  ...
  if (error) { console.error("[Smaashallah] photo upload failed:", error.message, error); throw error; }
