...
export default function App() {
  ...
  const [reg, setReg] = useState({ date: todayStr(), time: "", name: "" });
  ...
  const regByDate = useMemo(() => {
    ...
    Object.values(g).forEach((a) => a.sort((x, y) => (x.time || "").localeCompare(y.time || "") || x.name.localeCompare(y.name)));
    return g;
  }, [signups]);
  ...
  const addSignup = async () => {
    ...
    if (!name || !reg.date || !reg.time) { setRegMsg("Add a name, date and start time."); return; }
    const latest = await loadKey(K_SIGNUPS, signups, true);
    const dup = latest.some((s) => s.name.toLowerCase() === name.toLowerCase() && s.date === reg.date && s.time === reg.time);
    const next = dup ? latest : [...latest, { id: uid(), date: reg.date, time: reg.time, name }];
    setSignups(next);
          ...
          <section>
            <div className="bd-reg-form">
              <div className="bd-reg-row">
                ...
                <label>Start time
                  <input type="time" className="bd-input" value={reg.time}
                    onChange={(e) => { setReg({ ...reg, time: e.target.value }); setRegMsg(""); }} />
                </label>
              </div>
            ...
            {regDates.length === 0 ? (
              ...
              regDates.map((date) => (
                <div key={date} className="bd-reg-day">
                  ...
                  <ul className="bd-reg-list">
                    {regByDate[date].map((s) => (
                      <li key={s.id} className="bd-reg-item">
                        <span className="bd-reg-time">{fmtTime(s.time)}</span>
...
.bd-reg-item{display:flex;align-items:center;gap:10px;background:var(--panel);border:1.5px solid var(--line);border-radius:10px;padding:9px 11px;}
.bd-reg-time{font-family:'Barlow Semi Condensed',sans-serif;font-weight:700;font-size:13px;color:var(--accent);min-width:64px;}
.bd-reg-name{flex:1;font-weight:500;font-size:14px;}
.bd-reg-item button{border:none;background:transparent;color:var(--muted);cursor:pointer;display:flex;padding:2px;border-radius:5px;}
