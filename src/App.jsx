...
import {
  ...
  Lock, LockOpen, ChevronDown, ChevronUp, KeyRound, Camera
} from "lucide-react";
...
export default function App() {
      ...
      setRounds((rs) =>
        rs.map((r, i) =>
          i !== rIdx ? r : {
            ...r,
            matches: r.matches.map((m, j) =>
              j !== mIdx ? m : { ...m, photos: [...(m.photos || []), photo] }
            ),
          }
        )
      );
  ...
  const removePhoto = (rIdx, mIdx, pIdx) => {
    setRounds((rs) =>
      rs.map((r, i) =>
        i !== rIdx ? r : {
          ...r,
          matches: r.matches.map((m, j) => {
            ...
            const [gone] = photos.splice(pIdx, 1);
            if (gone) deletePhoto(gone.path);
            return { ...m, photos };
          }),
        }
      )
                    ...
                    {r.matches.map((m, mIdx) => {
                      ...
                      return (
                        <div key={mIdx} className="bd-court">
                          ...
                          {((m.photos && m.photos.length) || canEdit) && (
